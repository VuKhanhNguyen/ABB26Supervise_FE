...
import useAuthStore, { AuthState } from "@/shared/store/useAuthStore";
import useToastStore, { ToastState } from "@/shared/store/useToastStore";
...
export function ProfileScreen() {
  ...
  const scheduleTestNotification = async () => {
    showToast(
      "Bắt đầu đếm ngược 5 giây demo. Hãy thoát app ra Home ngay!",
      "info",
    );
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: 'Hệ thống phát hiện linh kiện "Dầu máy" sắp đến hạn. Vui lòng kiểm tra!',
        data: { data: "goes here" },
        sound: true,
        priority: "max",
      },
    ...
    <SafeAreaView className="flex-1 bg-background" edges={["top", "bottom"]}>
      ...
      >
        ...
        <View className="mb-10 items-center">
          ...
          <Text className="text-on-surface-variant font-medium text-sm mb-3">
            {user?.email}
          </Text>

          <View className="flex-row items-center mt-1">
            <Text className="text-primary font-headline text-lg font-medium mr-2">
              {user?.current_odo.toLocaleString() || "0"}
            </Text>
            <Text className="text-on-surface-variant font-bold text-[10px] tracking-widest uppercase">
              Tổng ODO km
            </Text>
          </View>
        </View>
        ...
        <View className="flex-row gap-4 mb-8">
          <View className="flex-1 bg-surface-container rounded-xl p-4 border-t border-primary/10 shadow-sm shadow-black justify-between">
            <Text className="text-on-surface-variant font-bold text-[10px] tracking-[0.2em] uppercase">
              TB Hàng ngày
            </Text>
          ...
          <View className="flex-1 bg-surface-container rounded-xl p-4 border-t border-secondary/10 shadow-sm shadow-black justify-between">
            ...
            <View className="mt-2 flex-row items-baseline">
              <Text className="font-headline text-2xl font-bold text-secondary">
                PRO
              </Text>
              <Text className="text-on-surface-variant text-[10px] ml-2 opacity-80 mt-1">
                Top 5% Pilots
              </Text>
            </View>
          </View>
        ...
        <View className="mb-10">
          <Text className="font-headline text-xs font-bold tracking-[0.2em] uppercase text-on-surface-variant mb-4 px-1">
            Xe Đã Đăng Ký
          </Text>
          <TouchableOpacity
            activeOpacity={0.9}
            className="bg-surface-container-low rounded-xl overflow-hidden shadow-lg shadow-black"
          >
            <View className="h-40 relative">
              <Image
                source={{
                  uri: "https://lh3.googleusercontent.com/aida-public/AB6AXuAfqgyjzY7wG0narZcyLLLRPOVUwQzaiCUqSxrJvMeyVC80dqaeIghOc5RVIeJfr9R73gJuI3cexjJ-v1GSEEjnASGBIAZoKbcVFEBgaWLtJUDR_eKBOf-_tOoEu2T_rkpzc6q6CNj0e5riOkhCtNSZO5HCe8jO4A7Luk9Yj4Z4Pf_cNsxJNX8V67A3vubHEdMIi4GyP1RCMw8uVVQbWQrGriT_SS2rcOrS-ZrZoU8BE2cqTUqdptpcKRo71fB1wY3MQpRccubLXQ",
                }}
              ...
              <View className="absolute bottom-4 left-4 border-l-2 border-primary pl-2">
                <Text className="text-primary text-[10px] font-bold tracking-widest uppercase mb-1 drop-shadow-md">
            ...
            <View className="p-4 flex-row justify-between items-center border-t border-outline-variant/10">
              <View className="flex-row">
                <View className="mr-6">
                  <Text className="text-[10px] text-on-surface-variant font-bold uppercase mb-1">
                    Firmware
                  </Text>
              ...
              <TouchableOpacity
                ...
                style={{
                  padding: 6,
                  borderRadius: 20,
                  backgroundColor: "rgba(152, 205, 242, 0.08)",
                }}
              >
